...
- **The compatibility matrix now lives here (`compatibility/`).** The contract is the neutral anchor every artifact speaks, so the matrix is judged and published in this repo: `aggregate.sh` fetches each artifact's `compatibility.json` feed (`feeds.txt` — epistola-suite as a server, valtimo-epistola-plugin as a client; best-effort, a not-yet-shipped feed is skipped), joins them with `compatibility-log.json` at the operation level (range-rule fallback, floor derived from the log), and `render.sh` writes `MATRIX.md`. The `Compatibility Matrix` workflow (daily schedule + manual + on log changes; seconds per run) regenerates and commits the deterministic outputs, giving the matrix memory and a stable URL. Verification that a published suite image really serves what it declares stays in `epistola-suite`.
