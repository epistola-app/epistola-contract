...
- **Consistent pagination across all list endpoints.** Every collection response now shares one `PageMeta` envelope (`page`, `size`, `totalElements`, `totalPages`) via a shared `spec/components/schemas/common.yaml#/PageMeta`, and every list `GET` accepts the shared `page`/`size` query parameters (`spec/components/parameters/pagination.yaml`). Previously only 4 of ~17 list responses were paginated (each redeclaring the fields inline) while the rest returned a bare `{ items }`. This is additive at the wire level — response bodies gain fields and the new query parameters are optional — so it is not a breaking change for consumers; the previously-paginated responses (tenants, consumers, generation jobs, documents) are unchanged on the wire. Bounded sub-resource lists (variant activations, stencil upgrade previews) remain unpaginated.
...
- **`oasdiff` now flattens `allOf` and shared parameters before diffing.** `make breaking` and the CI breaking-change check pass `--flatten-allof --flatten-params`, so the shared `PageMeta` envelope and path-level parameters are understood correctly instead of being misreported as removed properties.
