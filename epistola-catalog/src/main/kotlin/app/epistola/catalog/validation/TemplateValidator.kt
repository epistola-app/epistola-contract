...
object TemplateValidator {
    ...
    private fun validateBindings(
        ...
        document.nodes.values.sortedBy(Node::id).forEach { node ->
            ...
            validateParameterSchema(schema, "nodes.${node.id}.props.parameterSchemaSnapshot", findings)
    ...
    private fun validateParameterSchema(
        ...
        (properties as? Map<*, *>)?.entries?.sortedBy { it.key.toString() }?.forEach { (rawName, rawDefinition) ->
            ...
            if (!parameterNameRegex.matches(name)) findings.error(PARAMETER_NAME_INVALID, "$path.properties", "parameter name '$name' is invalid")
            if (name in reservedNames || reservedSuffixes.any(name::endsWith)) findings.error(PARAMETER_NAME_RESERVED, "$path.properties", "parameter name '$name' collides with a reserved scope name")
