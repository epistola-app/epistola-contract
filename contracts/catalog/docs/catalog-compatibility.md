...
| One wire version | Only catalog-wide `schemaVersion: 4` is supported. An older version is not accepted merely because its JSON can bind to the current classes. | Re-export the catalog from a current producer. A future version may add an explicit migration; none exists for a pre-v4 archive. |
...
| Complete stencil identity | Every stencil node must declare a valid `stencilId`, a positive exact `version`, and a boolean `isDraft`. | Re-save with a current authoring client. Portable catalog content must use `isDraft: false` and include the matching published stencil resource version. |
...
- Existing V1 catalog fingerprints remain accepted. New producers can use the
  corrected V2 fingerprint through `currentFingerprint`; the existing
...
`isDraft: true` while a stencil instance is being edited. A catalog is the
