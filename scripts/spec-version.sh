#!/usr/bin/env bash
# Single source of truth for parsing the spec version out of epistola-api.yaml.
#
# Usage:
#   scripts/spec-version.sh [--full|--api|--env]
...
SPEC_FILE="${SPEC_FILE:-$(cd "$(dirname "$0")/.." && pwd)/epistola-api.yaml}"
